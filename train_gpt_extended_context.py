...
class Hyperparameters:
    ...
    # --- Context extension (Bet 1) ---
    eval_extended_seq_len = int(os.environ.get("EVAL_EXTENDED_SEQ_LEN", 0))  # 0 = use train_seq_len
    rope_type = os.environ.get("ROPE_TYPE", "ntk")  # "ntk" (default, already in code) or "yarn"
    yarn_max_len = int(os.environ.get("YARN_MAX_LEN", 4096))  # target max length for YaRN scaling
    eval_batch_seqs = int(os.environ.get("EVAL_BATCH_SEQS", 32))  # batch size for sliding eval
    # --- Artifact-aware MDL (Bet 2) ---
    mdl_smooth_lambda = float(os.environ.get("MDL_SMOOTH_LAMBDA", 0.0))  # 0 = disabled
    mdl_smooth_interval = int(os.environ.get("MDL_SMOOTH_INTERVAL", 1))  # apply every N steps
    mdl_probe_interval = int(os.environ.get("MDL_PROBE_INTERVAL", 200))  # measure compressed size every N steps
...
class Rotary(nn.Module):
    def __init__(self, dim: int, base: float = 10000.0, train_seq_len: int = 1024,
                 rope_dims: int = 0, rope_type: str = "ntk", yarn_max_len: int = 4096):
        super().__init__()
        self.dim = dim
        self.base = base
        ...
        # Apply YaRN frequency scaling at init time if requested
        if rope_type == "yarn" and yarn_max_len > train_seq_len:
            scale = train_seq_len / yarn_max_len
            freq_idx = torch.arange(0, self.rope_dims, 2, dtype=torch.float32)
            ramp = torch.clamp((freq_idx / self.rope_dims - 0.25) / 0.75, 0.0, 1.0)
    ...
    def forward(self, seq_len: int, device: torch.device, dtype: torch.dtype) -> tuple[Tensor, Tensor]:
        if (
            ...
            if self.rope_type == "yarn":
                # YaRN: frequencies already scaled at init, just use them directly
                inv_freq = self.inv_freq.to(device)
            elif seq_len > self.train_seq_len:
                # NTK-aware scaling (original behavior)
        ...
        if rope_dims > 0:
            ...
            for block in self.blocks:
                ...
                block.attn.rotary = Rotary(head_dim, base=rope_base, train_seq_len=1024,
                                           rope_dims=rope_dims, rope_type=getattr(self, 'rope_type', 'ntk'),
                                           yarn_max_len=getattr(self, 'yarn_max_len', 4096))
...
class _HessianGPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
                 ...
                 rope_type="ntk", yarn_max_len=4096):
        super().__init__()
        self.rope_type = rope_type
        self.yarn_max_len = yarn_max_len
        self.tie_embeddings = tie_embeddings
...
def main() -> None:
    ...
    while True:
        ...
        # --- Bet 2: LZMA-aware smoothness regularization ---
        # Penalize inter-weight differences along serialization axis during warmdown.
        # Smoother weight sequences → runs of similar values → better LZMA compression.
        if args.mdl_smooth_lambda > 0 and scale < 1.0 and step % args.mdl_smooth_interval == 0:
            lam = args.mdl_smooth_lambda * (1.0 - scale)  # ramp up as LR decays
            with torch.no_grad():
                for p in base_model.parameters():
                    if p.grad is not None and p.ndim == 2 and p.size(1) > 1:
                        # Gradient toward smoother weight sequences (L2 on adjacent diffs)
                        diff = p[:, 1:] - p[:, :-1]
                        p.grad[:, 1:] += lam * diff
                        p.grad[:, :-1] -= lam * diff
        ...
        # --- Bet 2: Periodic compressed-size probe ---
        if (args.mdl_smooth_lambda > 0 and scale < 1.0
                and args.mdl_probe_interval > 0 and step % args.mdl_probe_interval == 0):
            with torch.no_grad():
                # Sample first MLP up-projection layer as proxy for full model
                sample_w = None
                ...
                if sample_w is not None:
                    s = sample_w.abs().amax(dim=1, keepdim=True) / 31
                    q = torch.round(sample_w / (s + 1e-10)).clamp(-31, 31).to(torch.int8)
                    packed = q.cpu().numpy().tobytes()
                    import lzma as _lzma
                    compressed = _lzma.compress(packed, preset=6)  # preset=6 for speed (vs 9)
                    bpp = len(compressed) * 8 / q.numel()
                    log0(f"mdl_probe step:{step} bits_per_param:{bpp:.3f} "
                         f"compressed_kb:{len(compressed)/1024:.1f} lr_scale:{scale:.4f}")
    ...
    hessian_model = _HessianGPT(
        ...
        rope_type=args.rope_type, yarn_max_len=args.yarn_max_len,
    ).to(device).bfloat16()
    ...
    # --- Extended context evaluation (Bet 1) ---
    ext_sl = args.eval_extended_seq_len
    if ext_sl > 0 and ext_sl != sw_seq_len and ext_sl > args.eval_stride:
        log0(f"extended_context:start seq_len={ext_sl} stride={args.eval_stride} "
             f"batch_seqs={args.eval_batch_seqs} rope_type={args.rope_type}")
        # Need to recompile for the new seq_len shape (dynamic=False locks to shape)
        torch.cuda.synchronize()
        t_ext = time.perf_counter()
        ext_val_loss, ext_val_bpb = eval_val_sliding(
            args, eval_model, rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            stride=args.eval_stride,
            eval_seq_len=ext_sl,
            batch_seqs=min(args.eval_batch_seqs, 16),  # reduce batch for memory
        )
        torch.cuda.synchronize()
        log0(
            f"extended_context val_loss:{ext_val_loss:.4f} val_bpb:{ext_val_bpb:.4f} "
            f"seq_len:{ext_sl} stride:{args.eval_stride} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_ext):.0f}ms"
        )
        log0(f"extended_context_exact val_loss:{ext_val_loss:.8f} val_bpb:{ext_val_bpb:.8f}")
    if distributed:
