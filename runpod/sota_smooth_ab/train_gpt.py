...
class Hyperparameters:
    ...
    val_tokens_max = int(os.environ.get("VAL_TOKENS_MAX", 0))  # 0 = full validation split
...
def main() -> None:
    ...
    smooth_tensors: list[tuple[str, Tensor]] = []
    if args.smooth_lambda > 0:
        smooth_tensors = [
            ("qo_bank", base_model.qo_bank),
            ("kv_bank", base_model.kv_bank),
            ("mlp_up_bank", base_model.mlp_up_bank),
            ("mlp_down_bank", base_model.mlp_down_bank),
        ]
        smooth_matrix_count = sum(int(tensor.shape[0]) for _, tensor in smooth_tensors)
        log0(
            f"smooth_lambda:{args.smooth_lambda} smooth_tensor_groups:{len(smooth_tensors)} "
            f"smooth_matrices:{smooth_matrix_count}"
        )
    ...
    while True:
        ...
        if smooth_tensors and scale < 1.0:
            smooth_loss = torch.zeros((), device=device)
            for _, tensor in smooth_tensors:
